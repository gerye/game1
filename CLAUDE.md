...
- **无尽推演**（唯一推演模式，快速推演已删除）：3年大循环（114步），结构：
  ...
  - 武道会奖励：冠军 SS，亚军 S
  - 排位赛奖励：冠军 SSS，亚军 SS
  - `season` 步骤：静默推进时节（含攻城AI），不触发 UI 面板
...
- **灭门战**：触发条件=某门派关联3大城全被他方占领；进攻方声望清零；防守方全员参战；进攻胜则内战决出最终赢家，赢家取HQ，防守方弟子半数归胜者半数流散
...
- 单挑队列流程：确认 → 逐场战斗 → 重伤状态写入 → 攻城结算完整走通
...
3. **攻城战可视化** — 攻城目前随机50%后台结算，未接入真实战场
4. **世界地图缩放/平移范围** — 地图半径40，HQ 在角落可能需要滚动才能看到
